  ...
  async testAllUsersWithOverdue() {
    ...
    try {
      const usersWithOverdue = await this.query(`
        ...
        WHERE ss.status = 'Pendente' AND ss.session_date < CURRENT_DATE
    ...
    try {
      ...
      const stats = await Promise.all([
        ...
        this.query(`
          ...
          WHERE status = 'Pendente' AND session_date < CURRENT_DATE
        `)
      ]);
      ...
      const complexResult = await this.query(`
        SELECT 
          ...
          COUNT(CASE WHEN ss.status = 'Pendente' AND ss.session_date < CURRENT_DATE THEN 1 END) as overdue,
          COUNT(CASE WHEN ss.status = 'Pendente' AND ss.session_date >= CURRENT_DATE THEN 1 END) as future,

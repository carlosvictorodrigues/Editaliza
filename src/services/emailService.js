...
Equipe Editaliza
    ...
    /**
     * Generate HTML email template for welcome with credentials
     */
    generateWelcomeWithCredentialsHTML(userName, email, password, planType, appUrl) {
        return `
<!DOCTYPE html>
<html lang="pt-BR">
<head>
...
<body>
    ...
    /**
     * Generate plain text version for welcome with credentials
     */
    generateWelcomeWithCredentialsText(userName, email, password, planType, appUrl) {
        return `
...
Sua conta ${planType} foi criada e está pronta para uso.
...
- Guarde bem estes dados
- Recomendamos alterar sua senha no primeiro acesso
- Se você não fez esta compra, contate-nos imediatamente
...
Equipe Editaliza
    ...
    /**
     * Send welcome email with login credentials for new registrations
     */
    async sendWelcomeEmailWithCredentials(email, userName, password, planType = 'Premium') {
        try {
            // If email service is not configured, simulate sending
            if (!this.isConfigured || !this.transporter) {
                console.log('\n📧 SIMULAÇÃO DE E-MAIL - DADOS DE ACESSO');
                console.log('═'.repeat(60));
                console.log(`Para: ${email}`);
                console.log(`Nome: ${userName || 'Cliente'}`);
            ...
            const appUrl = process.env.APP_URL || 'https://app.editaliza.com.br';
            const htmlContent = this.generateWelcomeWithCredentialsHTML(userName, email, password, planType, appUrl);
            const textContent = this.generateWelcomeWithCredentialsText(userName, email, password, planType, appUrl);
            ...
            const mailOptions = {
                ...
                to: email,
                subject: '🎉 Bem-vindo ao Editaliza - Seus dados de acesso estão aqui!',
                text: textContent,
                html: htmlContent,
                priority: 'high',
                headers: {
                    'X-Mailer': 'Editaliza Access System',
                    'X-Priority': '1'
                },
                attachments: [{
                    filename: 'logo.png',
            ...
            console.log('✅ Email com dados de acesso enviado com sucesso');
            console.log(`📧 Para: ${email}`);
            console.log(`🔑 Login: ${email}`);
            console.log(`🔐 Senha: ${password}`);
            console.log(`📝 Message ID: ${result.messageId}`);
            ...
            // Log detailed error for debugging but don't expose to client
            if (process.env.NODE_ENV === 'development') {
                console.error('Welcome credentials email error details:', error);
            }

            // Fallback to simulation if email fails
            console.log('\n📧 FALLBACK - SIMULAÇÃO DE DADOS DE ACESSO');
            console.log('═'.repeat(60));
            console.log(`Para: ${email}`);
            console.log(`Login: ${email}`);
            console.log(`Senha: ${password}`);

    ...
    async getStudyStreak(planId) {
        const query = `
            ...
            SELECT 
                ...
                CASE 
                    WHEN cs.streak_end = CURRENT_DATE THEN 1
                    ELSE 0
                END as is_active
    ...
    async getStudyTimeDistribution(planId) {
        const query = `
            WITH time_slots AS (
                SELECT 
                    CASE 
                        WHEN CAST(EXTRACT(HOUR FROM session_date) AS INTEGER) < 6 THEN 'madrugada'
                        WHEN CAST(EXTRACT(HOUR FROM session_date) AS INTEGER) < 12 THEN 'manhã'
                        WHEN CAST(EXTRACT(HOUR FROM session_date) AS INTEGER) < 18 THEN 'tarde'
                        ELSE 'noite'
                    END as period,
                    COUNT(*) as session_count,
            ...
            weekday_distribution AS (
                SELECT 
                    CASE CAST(EXTRACT(DOW FROM session_date) AS INTEGER)
                        WHEN 0 THEN 'Domingo'
                        WHEN 1 THEN 'Segunda'
                        WHEN 2 THEN 'Terça'
                        WHEN 3 THEN 'Quarta'
                        ...
                        WHEN 6 THEN 'Sábado'
                    END as weekday,
                    CAST(EXTRACT(DOW FROM session_date) AS INTEGER) as weekday_num,
                    COUNT(*) as session_count,
                    SUM(time_studied_seconds) as total_seconds
                FROM study_sessions
                WHERE study_plan_id = $1
    ...
    async getProgressComparison(planId, days = 7) {
        const query = `
            ...
            previous_period AS (
                ...
                FROM study_sessions
                WHERE study_plan_id = $1
                    AND session_date > CURRENT_DATE - INTERVAL '1 day' * ($2 * 2)
                    AND session_date <= CURRENT_DATE - INTERVAL '1 day' * $2
            )
            SELECT 
    ...
    async getDashboardMetrics(planId, userId) {
        const query = `
            SELECT 
                ...
                (SELECT COUNT(*) FROM topics t 
                 JOIN subjects s ON t.subject_id = s.id 
                 WHERE s.study_plan_id = $1 AND t.status = 'completed') as completed_topics,
                ...
                (SELECT ROUND(
                 ...
                 FROM study_sessions 
                 WHERE study_plan_id = $1 
                   AND session_date >= CURRENT_DATE - INTERVAL '7 days') as weekly_completion_rate
        `;

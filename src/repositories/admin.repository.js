...
class AdminRepository extends BaseRepository {
    ...
    async getSystemStatistics() {
        const query = `
            SELECT 
                ...
                (SELECT COUNT(*) FROM study_plans WHERE created_at > CURRENT_DATE - INTERVAL '7 days') as new_plans_7d,
    ...
    async getUpcomingExams(days = 30) {
        const query = `
            ...
            LEFT JOIN subjects s ON sp.id = s.study_plan_id
            LEFT JOIN topics t ON s.id = t.subject_id
            WHERE sp.exam_date BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '1 day' * $1
            GROUP BY sp.id, u.name, u.email
    ...
    async getRecentActivities(limit = 50) {
        const query = `
            WITH activities AS (
                ...
                FROM study_plans sp
                WHERE sp.created_at > CURRENT_DATE - INTERVAL '7 days'
                ...
                WHERE ss.completed_at > CURRENT_DATE - INTERVAL '7 days'
            ...
            const sessionsDeleted = await repo.delete(
                `DELETE FROM study_sessions 
                 WHERE status = 'completed' 
                   AND completed_at < CURRENT_DATE - INTERVAL '1 day' * $1`,
                [daysToKeep]
            );
    ...
    async getGrowthReport(months = 6) {
        const query = `
            WITH month_series AS (
                SELECT 
                    DATE_TRUNC('month', CURRENT_DATE) - INTERVAL '1 month' * (n-1) as month_start,
                    DATE_TRUNC('month', CURRENT_DATE) - INTERVAL '1 month' * (n-2) - INTERVAL '1 day' as month_end
                FROM (
            ...
            retention_cohorts AS (
                SELECT 
                    CASE 
                        ...
                        WHEN DATE_PART('day', 'now' - last_activity) > 30 THEN 'inactive'
                        WHEN DATE_PART('day', 'now' - last_activity) > 7 THEN 'at_risk'
                        ELSE 'active'

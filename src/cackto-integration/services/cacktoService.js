...
class CacktoService {
    constructor() {
        ...
        // API cliente só disponível com credenciais completas
        this.hasApiCredentials = !!(this.apiKey && this.secretKey);
        ...
        if (this.hasApiCredentials) {
            ...
            console.warn('⚠️  CacktoService: Rodando apenas com webhooks (sem API)');
            this.client = null;
        }
     ...
     */
    setupInterceptors() {
        // Só configurar interceptadores se o client existir
    ...
    async healthCheck() {
        try {
            // Se não temos cliente API, retornar status baseado apenas em webhook
            if (!this.hasApiCredentials || !this.client) {
                return {
                    status: 'webhook-only',
                    message: 'API credentials not configured, webhook-only mode',
            ...
            return {
                ...
                circuitBreakerState: this.circuitBreaker.state,
                lastCheck: new Date().toISOString(),
                mode: this.hasApiCredentials ? 'full-api' : 'webhook-only'
            };
        }
    }

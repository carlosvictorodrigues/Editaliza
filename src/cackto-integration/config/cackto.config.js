...
function validateConfig() {
    // Webhook secret é sempre obrigatório
    if (!process.env.CACKTO_WEBHOOK_SECRET) {
        throw new Error('Configuração CACKTO incompleta. Falta: CACKTO_WEBHOOK_SECRET');
    }

    // API credentials são opcionais se só webhook for usado
    const apiCredentialsMissing = !process.env.CACKTO_API_KEY || !process.env.CACKTO_SECRET_KEY;
    
    if (apiCredentialsMissing) {
        console.warn('⚠️  CACKTO rodando apenas com webhook. API Key e Secret Key não configuradas.');
        console.warn('   → Funcionalidades limitadas: apenas recebimento de webhooks');
    } else {
        console.log('✅ CACKTO totalmente configurado com API + Webhooks');
    }

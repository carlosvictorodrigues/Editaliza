...
(async () => {
  ...
  // Pre-set a dummy token to avoid frontend redirect to login
  await context.addInitScript(() => {
    try { localStorage.setItem('editaliza_token', 'qa-dummy-token'); } catch {}
  });
  ...
  page.on('response', async (response) => {
    ...
    if (url.startsWith(baseUrl)) {
      let body = null;
      try { if (status >= 400) body = await response.text(); } catch(_) {}
  ...
  try {
    ...
    if (tabCount > 0) {
      ...
      await page.waitForSelector('#avatar-gallery img.avatar-item', { timeout: 5000 }).catch(()=>{});
